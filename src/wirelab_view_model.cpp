...
namespace project
{
  ...
  WireLabViewModel::WireLabViewModel(QObject* parent)
      : QObject(parent), anomalyDetector_(gui_anomaly_config())
  {
  }

  bool WireLabViewModel::hasTopology() const noexcept { return topologyController_.has_topology(); }
  QString WireLabViewModel::topologyName() const { return QString::fromStdString(topologyConfiguration_.name); }
  ...
  QString WireLabViewModel::selectedType() const { return selectedType_; }
  QString WireLabViewModel::selectedId() const { return selectedId_; }
  QString WireLabViewModel::selectedSummary() const { return selectedSummary_; }
  QString WireLabViewModel::statusMessage() const { return statusMessage_; }
  bool WireLabViewModel::trafficRunning() const noexcept { return trafficRunning_; }
  QString WireLabViewModel::activeBackend() const { return activeBackend_; }
  QString WireLabViewModel::trafficResult() const { return trafficResult_; }
  QVariantList WireLabViewModel::metricsHistory() const { return metricsHistory_; }
  QVariantList WireLabViewModel::macTable() const { return macTable_; }
  QVariantList WireLabViewModel::portStates() const { return portStates_; }
  QVariantList WireLabViewModel::packetRows() const { return packetRows_; }
  ...
  void WireLabViewModel::saveTopology(const QString& path)
  {
    ...
    for (const auto& node : topologyConfiguration_.nodes)
      output << "  - { id: " << yaml_quote(node.id) << ", type: "
             << (node.type == TopologyNodeType::Switch ? "switch" : "host") << " }\n";
    output << "\nlinks:\n";
  ...
  void WireLabViewModel::rebuildTopologyModels()
  {
    ...
    const auto hostCount = static_cast<size_t>(std::count_if(
        topologyConfiguration_.nodes.begin(), topologyConfiguration_.nodes.end(),
  ...
  {
    const auto found = std::find_if(topologyConfiguration_.nodes.begin(), topologyConfiguration_.nodes.end(),
                                    [&id](const TopologyNode& node) { return node.id == id.toStdString(); });
    if (found == topologyConfiguration_.nodes.end()) return;
    selectedType_ = QStringLiteral("node");
    selectedId_ = id;
    selectedFirst_ = id;
    selectedSecond_.clear();
    selectedSummary_ = QStringLiteral("%1 node · %2 connected link(s)")
                           .arg(QString::fromLatin1(to_string(found->type)))
                           .arg(std::count_if(topologyConfiguration_.links.begin(), topologyConfiguration_.links.end(),
                                              [&found](const TopologyLink& link) {
                                                return link.from == found->id || link.to == found->id;
                                              }));
  ...
  {
    const auto found = std::find_if(topologyConfiguration_.links.begin(), topologyConfiguration_.links.end(),
    ...
    if (found == topologyConfiguration_.links.end()) return;
  ...
  void WireLabViewModel::addNode(const QString& id, const QString& type)
  {
    ...
    if (edited.name.empty()) edited.name = "untitled-lab";
    edited.nodes.push_back({ id.trimmed().toStdString(),
                             type.compare(QStringLiteral("switch"), Qt::CaseInsensitive) == 0
  ...
  {
    TopologyConfiguration edited = topologyConfiguration_;
    edited.links.push_back({ from.trimmed().toStdString(), to.trimmed().toStdString(),
                             std::chrono::milliseconds(latencyMs) });
  ...
  {
    if (selectedType_.isEmpty()) return;
    TopologyConfiguration edited = topologyConfiguration_;
    if (selectedType_ == QStringLiteral("node"))
    {
      const auto id = selectedFirst_.toStdString();
      edited.nodes.erase(std::remove_if(edited.nodes.begin(), edited.nodes.end(),
                                        [&id](const TopologyNode& node) { return node.id == id; }),
                         edited.nodes.end());
      edited.links.erase(std::remove_if(edited.links.begin(), edited.links.end(),
                                        [&id](const TopologyLink& link) {
    ...
    else
    {
      ...
      edited.links.erase(std::remove_if(edited.links.begin(), edited.links.end(),
                                        [&first, &second](const TopologyLink& link) {
                                          return (link.from == first && link.to == second) ||
                                                 (link.from == second && link.to == first);
                                        }),
                         edited.links.end());
    }
  ...
  void WireLabViewModel::resetSimulation()
  {
    ...
    anomalyRows_.clear();
    for (const auto& node : topologyConfiguration_.nodes)
      if (node.type == TopologyNodeType::Host) portCounters_.try_emplace(node.id);
    portStates_.clear();
    trafficResult_ = QStringLiteral("Ready. Configure traffic and press Start.");
    emit telemetryChanged();
  }

  void WireLabViewModel::startTraffic(const QString& scenario, int packetsPerTick, int frameSize,
  ...
  {
    if (!trafficRunning_ || !trafficGenerator_) return;
    std::vector<std::string> hosts;
    std::string switchId;
    for (const auto& node : topologyConfiguration_.nodes)
    {
      if (node.type == TopologyNodeType::Host) hosts.push_back(node.id);
      else switchId = node.id;
    }
    ...
    {
      const auto hostIndex =
          (tickSequence_ * static_cast<uint64_t>(packetsPerTick_) + index) % hosts.size();
      ...
      const auto link = std::find_if(topologyConfiguration_.links.begin(), topologyConfiguration_.links.end(),
      ...
      const auto linkDecision =
          topologyController_.evaluate_link(link->from, link->to, frames[index].size(), portArrival);
      ...
      ++latencySamples;
      delivered.push_back(
          { frames[index].data(), frames[index].size(), static_cast<uint32_t>(hostIndex) });
    }
  ...
  void WireLabViewModel::rebuildTelemetryModels(const AnalysisBatch& analysis, uint64_t tickBytes,
                                                 uint64_t tickDropped, double throughputMbps,
                                                 double averageLatencyMs)
  {
    const double lossPercent = packetsPerTick_ == 0 ? 0.0
                                                    : 100.0 * static_cast<double>(tickDropped) / packetsPerTick_;
    metricsHistory_.append(QVariantMap{{ "sequence", static_cast<qulonglong>(tickSequence_) },
                                       { "throughputMbps", throughputMbps },
                                       { "latencyMs", averageLatencyMs },
                                       { "lossPercent", lossPercent }});
    while (metricsHistory_.size() > 60) metricsHistory_.removeFirst();
    ...
    for (size_t index = 0; index < packetLimit; ++index)
    {
      ...
      if (packet.ingress_port < hosts.size()) ingress = QString::fromStdString(hosts[packet.ingress_port]);
      const auto sourceMac = packet.source_mac.to_string();
      learnedMacPorts_[sourceMac] = ingress.toStdString();
      packetRows_.append(QVariantMap{{ "source", QString::fromStdString(sourceMac) },
    ...
    for (const auto& [mac, port] : learned)
      macTable_.append(QVariantMap{{ "mac", QString::fromStdString(mac) },
                                   { "port", QString::fromStdString(port) }});

    portStates_.clear();
    for (const auto& node : topologyConfiguration_.nodes)
    {
      if (node.type != TopologyNodeType::Host) continue;
      const auto& counters = portCounters_[node.id];
      portStates_.append(QVariantMap{{ "id", QString::fromStdString(node.id) },
    ...
    const auto anomalies = anomalyDetector_.evaluate(analysis, tickSequence_ * 500'000'000ULL);
    anomalyRows_.clear();
    for (const auto& anomaly : anomalies)
      anomalyRows_.append(QVariantMap{{ "type", anomaly_name(anomaly.type) },
    ...
    trafficResult_ =
        QStringLiteral("%1 packets generated · %2 delivered bytes · %3 dropped · %4 Mbps · %5 ms average")
            .arg(totalPackets_)
            .arg(totalBytes_)
            .arg(totalDropped_)
  ...
  void WireLabViewModel::applySelectedFault(int latencyMs, double lossPercent, bool blackhole)
  {
    ...
    const auto result = selectedType_ == QStringLiteral("link")
                            ? topologyController_.set_link_fault(selectedFirst_.toStdString(),
                                                                 selectedSecond_.toStdString(), configuration)
                            : topologyController_.set_port_fault(selectedFirst_.toStdString(), configuration);
    setStatus(result ? QStringLiteral("Applied fault to %1").arg(selectedId_)
  ...
  {
    const bool cleared = secondEndpoint.isEmpty()
                             ? topologyController_.clear_port_fault(firstEndpoint.toStdString())
                             : topologyController_.clear_link_fault(firstEndpoint.toStdString(),
                                                                    secondEndpoint.toStdString());
    setStatus(cleared ? QStringLiteral("Cleared fault.")
                      : QStringLiteral("The selected fault was no longer active."));
    rebuildFaultModel();
  }
  ...
  void WireLabViewModel::rebuildFaultModel()
  {
    ...
    if (faults)
    {
      for (const auto& fault : faults.value())
      {
        ...
        activeFaults_.append(QVariantMap{
            { "first", QString::fromStdString(fault.first_endpoint) },
            { "second", QString::fromStdString(fault.second_endpoint) },
            { "target", link ? QStringLiteral("%1 ↔ %2").arg(QString::fromStdString(fault.first_endpoint),
                                                              QString::fromStdString(fault.second_endpoint))
                              : QString::fromStdString(fault.first_endpoint) },
            { "kind", link ? QStringLiteral("Link") : QStringLiteral("Port") },
            { "latencyMs", static_cast<qlonglong>(
                               std::chrono::duration_cast<std::chrono::milliseconds>(
                                   fault.configuration.latency).count()) },
